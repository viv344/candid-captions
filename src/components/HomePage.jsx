...
export default function HomePage(props) {
  ...
  return (
    <main className='flex-1  p-4 flex flex-col gap-3 text-center sm:gap-4  justify-center pb-20'>
        <h1 className=' font-semibold text-5xl sm:text-6xl md:text-7xl text-rose-400'>
            Candid<span className='text-cyan-400 bold'>Captions</span>
        </h1>
        ...
        <button onClick={recordingStatus==='recording' ? stopRecording:startRecording} className='flex specialBtn px-4 py-2 rounded-xl items-center text-base justify-between gap-4 mx-auto w-72 max-w-full my-4'>
            <p className=' font-semibold text-rose-400'>{recordingStatus ==='inactive'? 'Record...':'Stop Recording...'}</p>
        ...
        <input onChange={(e)=> {
            ...
            setFile(tempFile)
        }} className='hidden' type="file" accept='.mp3,.wav' /></label> an mp3 File
        </p>
        <p className='italic text-slate-400'>Transcription Services You Can Trust! </p>
